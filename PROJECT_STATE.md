...
- 重复判断结构化细化 V2：已在本地完成代码、单测、构建和小程序结构校验，新增 `pocketPresence`、`pocketPosition`、`chestMarkPresence`、`chestMarkType`、`chestMarkPosition`、`chestMarkText` 六个结构化字段，用来区分“同色同版型但胸前细节不同”的衣物，避免把“黑色短袖 + 小字母”和“黑色短袖 + 胸前口袋标签”误判成重复。
- 当前状态仍是“本地完成，待服务器拉取 + 微信开发者工具体验版验收”，所以暂时还不能写成“已上线验收通过”。
